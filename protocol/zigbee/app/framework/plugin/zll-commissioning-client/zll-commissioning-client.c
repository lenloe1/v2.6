...
static EmberStatus startScan(uint8_t purpose)
{
  ...
  if (touchLinkInProgress()) {
    ...
    currentChannel = emberGetLogicalChannel();
...
static boolean isTouchlinkPermitted(const EmberZllNetwork *networkInfo)
{
  ...
  if (status == EMBER_NOT_JOINED) { // unknown node type: not joined, joining, or leaving
    ...
    debugPrintln("isTouchlinkPermitted: node type = %d, security bitmask = %2X,flags = %2X",
                 nodeType, securityState.bitmask, emAfZllFlags);
  }
  ...
  if (touchLinkInProgress()) {
    ...
    if (scanStatus != EMBER_SUCCESS) {
      ...
      if ((scanForTouchLink() || scanForDeviceInformation())
        ...
        // The following call will return an error if the current 'Rx On' duration is greater
        // than the duration that we are requesting, therefore we won't check the status here.
...
void emAfZllAbortTouchLink(EmberAfZllCommissioningStatus reason)
{
  ...
  if (emberEventControlGetActive(emberAfPluginZllCommissioningClientTouchLinkEventControl)) {
    ...
    if (emAfZllNetwork.numberSubDevices != 1) {
      // We need to be careful here, since there may have been an existing
      // Rx On operation in progress before we started touchlinking -
      // however, since we are currently using a duration of 0 as a
      // cancel operation, we need to specify a non-zero duration here.
      emberZllSetRxOnWhenIdle(1); // restore original Rx On mode
    }
    sendIdentifyRequest(0x0000); // exit identify mode
  }
  EmberStatus status = emberSetRadioChannel(currentChannel);
  if (status != EMBER_SUCCESS) {
    emberAfAppPrintln("%p%p0x%x",
                      "Error: ",
